...
from analytics.data import espn
...
def reconcile_picks(yesterday: date) -> None:
    ...
    from ESPN and inserts them into nba_player_stats first.
    """
    ...
    if not has_stats:
        ...
        # Look up game ext_ids from DB first, so ESPN's summary endpoint is
        # only hit for games we already know about.
        game_rows = (
            supabase.table("games")
            .select("ext_id")
            .eq("game_date", yest_str)
...
def _fetch_and_insert_box_scores(
    ...
    """
    Fetch box scores for target_date from ESPN's summary endpoint and insert
    rows into nba_player_stats.

    If game_ext_ids is provided (ESPN event ids from the DB), use them
    directly. Otherwise discover them via ESPN's scoreboard for target_date.

    Team/player identity is resolved by abbreviation/name, not ext_id — ESPN's
    ids don't correspond to nba_api's ids that older `games`/`players` rows
    were keyed on. This mirrors injury_check.py's proven-in-production
    approach to the same ESPN data source.
    """
    date_str = _date_str(target_date)

    if game_ext_ids is None:
        scoreboard = espn.fetch_scoreboard(date_str)
        game_ext_ids = [g["ext_id"] for g in scoreboard if g["completed"]]
        if not game_ext_ids:
            print(f"  No completed ESPN games found for {date_str}.")
            return
    ...
    # Load player ID map: (team_abbr, lowercased name) -> player_db_id.
    # Scoped to NBA active + inactive players so retired/traded players from
    # earlier in the season still resolve.
    player_rows = supabase.table("players").select("id,name,team").eq("league", "nba").execute()
    player_map: dict[tuple[str, str], int] = {
        ((r.get("team") or "").upper(), (r.get("name") or "").lower()): r["id"]
        for r in (player_rows.data or [])
    }

    team_rows = supabase.table("teams").select("id,abbreviation").eq("league_id", NBA_LEAGUE_ID).execute()
    team_map: dict[str, int] = {r["abbreviation"].upper(): r["id"] for r in (team_rows.data or [])}
    ...
    for ext_id in game_ext_ids:
        ...
        summary = espn.fetch_summary(ext_id)
        if summary is None:
            print(f"  WARNING: ESPN summary fetch failed for {ext_id}.")
            continue

        player_rows_from_api = espn.parse_boxscore(summary)
        if not player_rows_from_api:
            print(f"  WARNING: no box score data found for {ext_id}.")
            continue
        print(f"    {ext_id}: {len(player_rows_from_api)} player rows (ESPN)")

        # Team scores straight from the scoreboard competitor entries — more
        # reliable than summing parsed box-score points.
        try:
            header_competitors = ((summary.get("header") or {}).get("competitions") or [{}])[0].get("competitors") or []
            home_c = next((c for c in header_competitors if c.get("homeAway") == "home"), None)
            away_c = next((c for c in header_competitors if c.get("homeAway") == "away"), None)
            if home_c and away_c and game_db_id in game_teams:
                hs = int(home_c.get("score")) if home_c.get("score") is not None else None
                aws = int(away_c.get("score")) if away_c.get("score") is not None else None
        ...
        for r in player_rows_from_api:
            t_db_id = team_map.get(r["team_abbr"])
            p_db_id = player_map.get((r["team_abbr"], r["name"].lower()))
            if not p_db_id or not t_db_id:
                print(f"  WARNING: could not resolve player {r['name']!r} ({r['team_abbr']}) for game {ext_id}. Skipping.")
                continue
            stat_buffer.append({
                "game_id": game_db_id,
                "player_id": p_db_id,
...
def backfill_completed_games(up_to: date) -> None:
    ...
    between that date+1 and up_to (exclusive), fetch completed games from
    ESPN's scoreboard and their box scores via _fetch_and_insert_box_scores.
    ...
    # Load lookup map once (abbreviation -> team db id, since ESPN's team ids
    # don't correspond to nba_api's — see analytics/data/espn.py docstring)
    team_rows = (
        supabase.table("teams").select("id,abbreviation").eq("league_id", NBA_LEAGUE_ID).execute()
    )
    team_map = {r["abbreviation"].upper(): r["id"] for r in (team_rows.data or [])}
    ...
    while current < up_to:
        ...
        if not games_exist.data:
            # Need to fetch and insert game rows via ESPN's scoreboard
            scoreboard = espn.fetch_scoreboard(date_str)
            completed = [g for g in scoreboard if g["completed"]]
            ...
            for g in completed:
                home_db = team_map.get(g["home"]["abbr"])
                away_db = team_map.get(g["away"]["abbr"])
                if not home_db or not away_db:
                    print(f"  WARNING: team not in DB — home={g['home']['abbr']!r} away={g['away']['abbr']!r}")
                    continue
...
def get_slate(target_date: date) -> list[dict]:
    ...
    for target_date, backed by ESPN's scoreboard endpoint (scheduled +
    completed games, any date — including future slates for the
    slate-sync job).
    """
    date_str = _date_str(target_date)
    ...
    print("  No games in local DB. Trying ESPN scoreboard ...")

    # Team abbreviation -> db_id (ESPN's team ids don't correspond to
    # nba_api's, so identity is resolved by abbreviation — see espn.py docstring)
    team_rows = (
    ...
    scoreboard = espn.fetch_scoreboard(date_str)
    print(f"  ESPN scoreboard: {len(scoreboard)} game(s) returned.")

    games_to_insert: list[dict] = []
    for g in scoreboard:
        home_db_id = team_map.get(g["home"]["abbr"])
        away_db_id = team_map.get(g["away"]["abbr"])
    ...
    print(f"  ESPN scoreboard: {len(games_to_insert)} game(s) parsed.")

    if not games_to_insert:
        print(f"  WARNING: 0 game(s) found for {date_str}.")
        return []
