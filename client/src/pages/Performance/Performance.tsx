...
export default function Performance() {
  ...
  const [bottomView, setBottomView] = useState<BottomView>('streaks')
      ...
      <main className="flex-1 overflow-y-auto p-5 space-y-4">
        ...
        {!loading && !error && data && (
          <>
            ...
            {(() => {
              const PICK_FILTERS: { key: PickFilter; label: string }[] = [
                { key: 'all',     label: 'All' },
                { key: 'hit',     label: 'Hit' },
                { key: 'miss',    label: 'Miss' },
                { key: 'pending', label: 'Pending' },
              ]
              const countFor = (k: PickFilter) => {
                if (k === 'hit')     return data.recentPicks.filter(p => p.did_hit === true).length
                if (k === 'miss')    return data.recentPicks.filter(p => p.did_hit === false).length
                if (k === 'pending') return data.recentPicks.filter(p => p.did_hit === null).length
                return data.recentPicks.length
              }
                  ...
                  <div className="flex items-center border-b border-[#111]">
                    ...
                    <div className="flex-1 flex justify-end pr-4">
                      <span className="text-[9px] text-gray-700 font-condensed uppercase tracking-widest">
                        {bottomView === 'streaks' ? 'next-game continuation rate' : 'most recent first'}
                      </span>
                    </div>
                  </div>

                  {/* ── Streaks view */}
                  {bottomView === 'streaks' && <StreakPerformanceCard days={days} bare />}
